...
def upload_image(request):
    if request.method == "POST" and request.FILES.get("image"):
        ...
        watermarked_image_path = embed_watermark(
            image_name=image_file.name, hash_value=image_hash
        )
        sign_hash = encrypt_string(image_hash)

        try:
            uploaded_image = UploadedImage(
                image="uploads/" + image_file.name, hash=image_hash
            )
            uploaded_image.save()
            ...
            return JsonResponse(
                data={
                    "message": "The file seems to already exist in the database",
                    "error": str(e),
                },
                status=400,
            )
        ...
        try:
            ...
            response["signed_hash"] = (
                sign_hash  # Additional header to include the signed hash
            )
            return response
    ...
    return JsonResponse(
        {
            ...
            "file_exist_at_image": (
                "Correct" if request.FILES.get("image") else "incorrect"
            ),
        },
        status=400,
    )
...
def test(request):
    ...
    watermarked_image_path = embed_watermark(
        image_name=image_path, hash_value=original_hash
    )
    ...
    if original_hash == extracted_hash:
        return JsonResponse(
            {
                "message": "The watermark matches the original hash. Verification successful."
            }
        )
    else:
